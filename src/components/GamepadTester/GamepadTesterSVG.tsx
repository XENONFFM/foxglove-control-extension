...
export function GamepadTesterSVG({ gamepad }: { gamepad: GamepadState | null }): JSX.Element {
  ...
  const getButtonColor = (elementId: string): string => {
    ...
    if (buttonIndex != null && gamepad.buttons[buttonIndex] != null) {
      const value = gamepad.buttons[buttonIndex] as number | undefined;
      if (value != null) {
  ...
  // Stick calculations
  const lstickAxisX = gamepad.axes[0] ?? 0;
  const lstickAxisY = gamepad.axes[1] ?? 0;
  const lstickMagnitude = Math.sqrt(lstickAxisX * lstickAxisX + lstickAxisY * lstickAxisY);

  const rstickAxisX = gamepad.axes[2] ?? 0;
  const rstickAxisY = gamepad.axes[3] ?? 0;
  const rstickMagnitude = Math.sqrt(rstickAxisX * rstickAxisX + rstickAxisY * rstickAxisY);

  const lPressed = gamepad.buttons[10]?.valueOf() ?? 0;
  const rPressed = gamepad.buttons[11]?.valueOf() ?? 0;
